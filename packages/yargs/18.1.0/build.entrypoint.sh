#!/usr/bin/env bash
# Tier A factory build: yargs from git source via npm install + build + pack.
...
echo "[build.entrypoint] Checked out commit: ${actual_sha}"

echo "[build.entrypoint] Installing dependencies"
npm install --include=dev --ignore-scripts

echo "[build.entrypoint] Running prepare script"
npm run prepare --if-present

echo "[build.entrypoint] Packing tarball"
rm -f "${main_tgz}"
